...
const ChildFolder = (props) => {
    ...
    const gettingSData = (event,data) => {
        if (event.detail === 2) {
            props.changRout("module")
        }
        props.getRow(data);
    }
    const columns = [
            ...
            title: 'Test Name', field: 'name', render: (rowData) => {
                return (
                    <div>
                        ...
                        <Button onClick={(e) => { gettingSData(e,rowData) }} style={{ padding: "1.5vh 15%", border: "none" }} variant="outlined"
...
export default (withStyles({}, { withTheme: true })(ChildFolder));
